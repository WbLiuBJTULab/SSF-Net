...
class PositionalEmbedding(nn.Module):  # 利用初始位置进行正余弦编码
    def __init__(self, demb=256):
        ...
        self.register_buffer('inv_freq', inv_freq)  # 是一个频率数组，用于生成正弦和余弦波形，这些波形用于构建位置嵌入向量。

    # pos_seq =  pos_seq = torch.arange(seq_len-1, -1, -1.0)
    def forward(self, pos_seq, batch_size=2):
        sinusoid_inp = torch.ger(pos_seq, self.inv_freq)   # 位置序列 pos_seq生成正弦和余弦波形
        pos_emb = torch.cat([sinusoid_inp.sin(), sinusoid_inp.cos()], dim=-1)  # 将正弦和余弦波形拼接起来形成位置嵌入向量

        if batch_size is not None:
            return pos_emb[:, None, :].expand(-1, batch_size, -1)  #张量沿着批次维度扩展  这种扩展操作通常用于确保位置嵌入向量可以广播（broadcast）到与批次中每个样本相对应的位置。
        else:  # 如果我们的模型一次处理多个样本（即一个批次），我们需要为每个样本复制位置嵌入向量。
            return pos_emb[:, None, :]
...
class CrossAttention(nn.Module):
    ...
    def __init__(self, hidden_dim, pos = True, head = 4):
        ...
        self.pos_dim = 8  # 位置嵌入维度  4  4
        self.pos = pos  #是位置编码

        if self.pos:
            self.pos_en = PositionalEmbedding(self.pos_dim)  # 创建一个 PositionalEmbedding 实例 pos_en
        ...
        self.att = nn.MultiheadAttention(hidden_dim, head)  # 创建一个 nn.MultiheadAttention 实例 att，用于执行多头注意力操作。


    def forward(self, inputs, Q_in): # N,B,C

        batch_size = inputs.shape[1]
        seq_len = inputs.shape[0]  #numpy数组

        if self.pos:
            pos_input = torch.from_numpy(np.arange(seq_len)+1).cuda()  # 将 NumPy 数组转换为 PyTorch 张量    修改张量会影响 NumPy 数组  默认从0开始
            pos_input = self.pos_en(pos_input, batch_size)
            inputs_pos = torch.cat([inputs, pos_input], -1)  # N,B,C
        ...
        return out[0]  # 返回多头注意力层的输出的第一个元素，通常是经过注意力加权后的特征。
...
def gen_sample_grid(rois, grid_size=7, grid_offsets=(0, 0), spatial_scale=1.): #生成每个体素中心点的全局坐标，并将其转换为图像坐标系中的像素坐标。这是通过将3D体素网格映射到2D图像平面上的过程。
    faked_features = rois.new_ones((grid_size, grid_size))
    N = rois.shape[0]
    dense_idx = faked_features.nonzero()  # (N个, 2) [x_idx, y_idx]  获取非零元素的索引，这些索引对应于稀疏张量中非空的体素。
    dense_idx = dense_idx.repeat(N, 1, 1).float()  # (B, 7 * 7, 2)

    local_roi_size = rois.view(N, -1)[:, 3:5]  # 长宽
    local_roi_grid_points = (dense_idx ) / (grid_size-1) * local_roi_size.unsqueeze(dim=1) \
                      - (local_roi_size.unsqueeze(dim=1) / 2)  # (B, 7 * 7, 2)   1.归一化 2.每个roi坐标 3.减去每个体素大小的一半，得到基于局部中心的坐标 

    ones = torch.ones_like(local_roi_grid_points[..., 0:1])
    local_roi_grid_points = torch.cat([local_roi_grid_points, ones], -1) # 包含 z 坐标的完整点云张量。

    global_roi_grid_points = common_utils.rotate_points_along_z(
        local_roi_grid_points.clone(), rois[:, 6]
    ).squeeze(dim=1)                                              # rois[:, 6] 包含旋转角度信息
