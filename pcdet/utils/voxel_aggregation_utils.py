...
def get_voxel_indices_to_voxel_list_index(x_conv):
    ...
    """
    x_conv_indices = x_conv.indices # [17849,4]
    # Note that we need to offset the values by 1 since the dense representation has "0" to indicate an empty location
    x_conv_values = torch.arange(1, x_conv_indices.shape[0]+1, device=x_conv_indices.device) # [17849:1~17849]
    ...
    # indices索引非零元素所在位置（b，d，w，h），values为非零元素值，
    ...
    return x_conv_hash_table # [1,11,400,352]
...
def get_centroid_per_voxel(points, voxel_idxs, num_points_in_voxel=None):
    ...
    # unique去除重复点  centroid_voxel_idxs为去重后的体素点，unique_idx为旧列表中唯一值的在新列表中的出现位置，labels_count为旧列表中唯一值的频率值
...
def get_centroids_per_voxel_layer(points, feature_locations, multi_scale_3d_strides, voxel_size, point_cloud_range):
    ...
    points_valid = points[voxel_idxs_valid_mask] # 过滤掉超出点云可用范围的点
